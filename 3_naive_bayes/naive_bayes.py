...
Original file is located at
    https://colab.research.google.com/drive/1gIQp0EXBd0PbrIuhgubX-6AvjQFxJkzy
"""
...
class MyNaiveBayes:
    ...
    def fit(self, bows, labels):
        ...
        for c in range (self.num_classes):
          for i in range (num_batches):
            if (labels[i]==c):
              self.class_to_num_sentences[c]+=1   
              self.class_and_word_to_counts[c]=np.add(self.class_and_word_to_counts[c],bows[i])
    ...
    def get_prior(self):
        ...
        """
        num_batches=sum(self.class_to_num_sentences)
        prior = 1/num_batches*np.array(self.class_to_num_sentences)
        return prior
    def get_likelihood_with_smoothing(self):
        ...
        """
        k = 1
        num_batches= np.sum(self.class_to_num_sentences)
        likelihood = np.zeros((self.num_classes,self.num_vocab))
        total_vocab = np.sum(self.class_and_word_to_counts,axis=1)
        for c in range (self.num_classes):
          for w in range(self.num_vocab):
            likelihood[c, w]=(self.class_and_word_to_counts[c, w]+k)/(total_vocab[c]+k*self.num_vocab)
        return likelihood
    def predict(self, bows):
        ...
        """
        num_batches,num_vocab=np.shape(bows)
        posterior = np.zeros((num_batches,self.num_classes))
        for b in range (num_batches):
          for c in range (self.num_classes):
            posterior[b, c] = self.log_prior[c]+np.dot(bows[b],self.log_likelihood[c])
        L = np.argmax(posterior, axis=1)
        return L
    def _check(self):
        """Do not modify the code in this function."""
