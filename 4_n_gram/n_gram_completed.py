...
Original file is located at
    https://colab.research.google.com/drive/1Sr5MKOlS0HFWE_X9B5HQr-hXeOxy1t-2
"""
...
def add_sentence_tokens(sentences, n):
    ...
    if n>=2:
      for i in range(len(sentences)):
        result.append('<s> '*(n-1) + sentences[i] +  ' </s>'*(n-1))
    else:
      for i in range(len(sentences)):
        result.append('<s> ' + sentences[i] +  ' </s>')
    return result
...
def replace_unknown(tokens, cut_off):
    ...
    for i in range(len(tokens)):
      if  tokens[i] not in ['<s>','</s>'] and freq[tokens[i]] <= cut_off:
        tokens[i]= '<UNK>'
    return  tokens
...
class LanguageModel(object):
    ...
    def _convert_oov(self, ngram):
        ...
        for mask_i in self.masks:
          check = mask(ngram,mask_i)
          if check in self.model.keys():
            return check
        return check
    ...
    def perplexity(self, test):
        ...
        for ngram in test_ngrams:
          ngram = self._convert_oov(ngram)
          if ngram in self.model.keys():
            middle.append(self.model[ngram])
        log_prob = map(lambda x: math.log(x), middle)
        return math.exp(-1/length*sum(log_prob))
    ...
    def _best_candidate(self, prev, i, without=None):
        ...
        without.append('<UNK>')
        candidates = []
        for Xn, prob in self.model.items():
          if Xn[0:self.n - 1] == prev:
            candidates.append((Xn[-1],prob))
        candidates = filter(lambda x: x[0] not in without, candidates)
        def getKey(item):
          return item[1]
        candidates = sorted(candidates,key=getKey,reverse=True)
        if len(candidates) == 0:
          return ('</s>',1)
        elif (prev==()) or (prev[-1] == '<s>'):
          return candidates[i]
        else:
          return candidates[0]
