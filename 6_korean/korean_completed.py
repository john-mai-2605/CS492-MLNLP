...
Original file is located at
    https://colab.research.google.com/drive/1gB0OBRvOYUClJE7qnui1Skf99VwMRqgR
"""
...
class BytePairEncoder:
    ...
    def _build_subword_units(self, vocabs):
        ...
        def get_stats(vocabs):
            ...
            for word in vocabs.keys():
              ...
              for i in range(len(subwords)-1):
                pairs[(subwords[i],subwords[i+1])]+=vocabs[word] 
            return pairs
        ...
        def merge_vocab(pair, v_in):
            ...
            """
            v_out = {}
            long_word = '_ '+''.join(pair)
            long_word_token = '_ '+' '.join(pair)
            for key, value in v_in.items():
              if key == long_word_token:
                v_out[long_word] = value
              else:
                v_out[key] = value
            return v_out
...
def build_model(learning_rate,
    ...
    fw_multi_cell = tf.nn.rnn_cell.MultiRNNCell([
      tf.nn.rnn_cell.DropoutWrapper(
        tf.nn.rnn_cell.GRUCell(num_hidden),output_keep_prob=keep_prob,
